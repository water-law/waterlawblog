...
def _read_body(obj):
    ...
    cache.set("res" ,obj, timeout=3600*24)
...
def _http_call(the_url, method, authorization, **kw):
    ...
    try:
        if method == _HTTP_GET:
            resp = requests.get(http_url, headers=headers, params=kw, timeout=5)
        else:
            resp = requests.post(http_url, headers=headers, data=kw, timeout=5)
        r = JsonDict(resp.json())
